...
wn_render_summary <- function(region = NULL) {
  ...
  sql_command <- glue::glue_sql(
    ...
    SELECT
      (SELECT COUNT(twn_id)
       FROM {config$database}.{config$db_schema_00a}.[twn_all_notices]
       WHERE [twn_status_id] <> 7{school_region}) AS total_live_records,
      (SELECT COUNT(t.twn_date_id)
       FROM {config$database}.{config$db_schema_00a}.[twn_date_tracking] t
       LEFT JOIN {config$database}.{config$db_schema_00a}.[twn_all_notices] a ON t.twn_id = a.twn_id
