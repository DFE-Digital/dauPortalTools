...
SELECT c.quality_name
        ...
        ,[date_created]
        ,[last_checked]
     FROM {`config$database`}.{`config$schemas$db_schema_01a`}.[quality_list] l
     LEFT JOIN {`config$database`}.{`config$schemas$db_schema_01a`}.[quality_check] c ON c.quality_check_id = l.error_id
     LEFT JOIN {`config$database`}.{`config$schemas$db_schema_01a`}.[app_list] a ON l.app_id = a.app_id
  ...
  log_event(glue::glue("Created {sql_command}."), config)
